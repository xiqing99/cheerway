...
Ext.define("accountMgt.RfdPerStockRtListPanel",
{   
    extend : "smartOA.commonClass.DateRangeGridPanel",
    constructor : function(conf)
    { 
        
        Ext.define('RfdPerStockRt',{
        ...
        Ext.apply(this, {         
            ...
            editUrl: '',
            dataModel: 'RfdPerStockRt',  
            searchField: 'stockRtVchSeqNum',
            columns: [
              {xtype: 'rownumberer', align:'center', resizable:true, width: 40, header: '序号'},
              {text : '', dataIndex:'vchId', hidden: true},
              {text : "退货单号" , dataIndex : 'stockRtVchSeqNum', sortable : true, width : 120, align : 'center',
                    summaryType : 'count', 
                    summaryRenderer: function(value, summaryData, dataIndex) {
                        return '单据数  :  ' + value;
                    }                 
              },
              {text : "退货日期" , dataIndex : 'createdDate', filter:true,
              ...
              {text : "剩余应付金额" , dataIndex : 'remainedAmount', sortable : true, width : 150, align : 'center',
                    summaryType : 'sum', 
                    renderer:function(data, cell, record, rowIndex,columnIndex, store)
                    {
                        var currency = record.get('currencyName');
              ...
              {text : "付款期限" , dataIndex : 'deadlineDate', filter:true,
                renderer: Ext.util.Format.dateRenderer('Y-m-d'),sortable : true, width : 100, align : 'center'},
              {text : "付款日期" , dataIndex : 'paidOffDate', filter:true,
                renderer: Ext.util.Format.dateRenderer('Y-m-d'),sortable : true, width : 100, align : 'center'},
    ...
    openDetailsWin : function(id)
    {
             smartOA.util.genWindow({
                    title    : '应付款单',
                    loader: {
                            url: 'accountMgt/editRfdPerStockRt.js',
                            autoLoad: true,
                            scripts:true
                        },
                    width    : 800,
