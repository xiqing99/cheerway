...
Ext.define('smartOA.salesMgt.orderItemModel',{
    ...
    fields: [
         ...
         {name: 'totalPriceBeforeTax',  type: 'float'},                
         {name: 'tax',  type: 'float'},                 
...
//                    margins: '5 0 0 5'
            ...
            },            
            {
                ...
                {text : "税前总价" , dataIndex : 'totalPriceBeforeTax', sortable : true, width : 80, align : 'center',
                    ...
                    {
                         return record.get('unitPrice')*record.get('quantity');
                    },                   
                ...
                {text : "税额" , dataIndex : 'tax', width : 80, align : 'center', 
                    renderer : function(value, metaData, record, rowIdx, colIdx, store, view)
                    {
                         return record.get('unitPrice') * record.get('quantity')*record.get('taxRate') *0.01;
                    },                     
                    summaryType: function(records)
                    {
                        var total = 0;
                        ...
                        {
                            record = records[i];
                            total += record.get('unitPrice') * record.get('quantity')*record.get('taxRate') *0.01;
                        }
                        
                        return total;
                    },
                    summaryRenderer: function(value, summaryData, dataIndex) {
                        return currenyFormat(value);
                    }                
                },
                {text : "合计金额" , dataIndex : 'totalPrice', sortable : true, width : 80, align : 'center',
                    renderer : function(value, metaData, record, rowIdx, colIdx, store, view)
                    {
                         var ret = record.get('unitPrice') * record.get('quantity')*(100+record.get('taxRate')) *0.01;
                         return Ext.util.Format.usMoney(ret);
                    },                     
                    summaryType: function(records)
                    {
                        var total = 0;
                        ...
                        {
                            record = records[i];
                            total += record.get('unitPrice') * record.get('quantity')*(100+record.get('taxRate')) *0.01;
                        }
